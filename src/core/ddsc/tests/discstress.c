...
static uint32_t createwriter_subscriber (void *varg)
{
  ...
  for (int i = 0; i < N_READERS; i++)
  {
    ...
    CU_ASSERT_NEQ (rc == 0, 0);
    CU_ASSERT_NEQ ((mask & DDS_SUBSCRIPTION_MATCHED_STATUS) != 0, 0);
...
#define READ_LEN 3
  ...
  CU_ASSERT_NEQ (err == 0, 0);
  CU_ASSERT_NEQ (nwri >= (N_ROUNDS / 3) * N_READERS * N_WRITERS, 0);
